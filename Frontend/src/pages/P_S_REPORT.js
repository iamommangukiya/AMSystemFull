...
const P_S_REPORT = () => {
  ...
  const totalPayAmount = filteredData.reduce(
    (acc, item) =>
      acc +
      parseFloat(
        item.payAmount == "" || item.payAmount == "undefined"
          ? 0
          : item.payAmount
      ),
    0
  ...
  const totalDueAmount = filteredData.reduce(
    (acc, item) =>
      acc +
      parseFloat(
        item.dueAmount == "" || item.dueAmount == "undefined"
          ? 0
          : item.dueAmount
      ),
    0
            ...
            <div className="container">
              <div className="grid grid-cols-2">
                ...
                <div className="text-end">
                  <h1 className="text-xl font-bold">{mode} Report</h1>
                  <p className="text-lg">{filteredDate}</p>
                  <p className="text-xl font-bold py-5">
                    Total {mode} Amount: {formattedTotalSaleAmount}
                  </p>
                </div>
              </div>
            </div>
